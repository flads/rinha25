...
func processorDefaultHandler(w http.ResponseWriter, r *http.Request) {
	...
	parsedTime, err := time.Parse("2006-01-02T15:04:05.000Z", req.RequestedAt)
	if err != nil {
		http.Error(w, "Invalid requestedAt format", http.StatusBadRequest)
		return
	}

	p := Payment{
		Timestamp: parsedTime.UnixMicro(),
		Amount:    req.Amount,
	}

	log.Printf("Received default payment (converted): %+v", p)
...
func processorFallbackHandler(w http.ResponseWriter, r *http.Request) {
	...
	log.Printf("Received fallback payment (converted): %+v", p)
